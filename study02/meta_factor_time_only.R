...
# open data
data <- read.csv(file="./meta_data_time_only.csv",header=TRUE, sep=",")

data <- escalc(measure="ZCOR", ri=r_adj, ni=N, data=data, slab=paste(author, year, sep=", ")) 
#View(data)

# Mixed-effects analysis
res <- rma(yi, vi, mods = ~ factor(psychopathology_dummy_code) ,data=data) 
res 
...
dev.copy2eps(file="meta_funnelplot_metafor_psychopathology_term_TIME_ONLY.eps",width=7)
...
dev.print(postscript,"meta_forestplot_metafor_psychopathology_TIME_ONLY.eps", width = 7, horizontal=FALSE)